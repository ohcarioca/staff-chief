...
The library retains only Markdown and metadata in the local database and version 4 backups. Original files are not preserved. Import limits are 20 MiB per file and 2 million converted characters. Research conversations automatically capture all active notes and optional selected library documents at creation, without a confirmation step or an AI call. Each Send authorizes the question, recent history and retrieved excerpts from those fixed sources. Importing a file itself is not AI submission authorization. See [Conversational research](CONVERSATIONAL_RESEARCH.md).
